...
- **Final-review fixes follow-ups (2026-07-22):** three items deliberately deferred rather than folded into the review-fix pass — (1) the collector's `GET /issues` level filter (`apps/signals/src/routes/issues.routes.ts`) is applied to the `enriched` array *after* pagination/`total` are already computed from the unfiltered query, so `total` (and the page count derived from it) over-counts whenever `level` is set — needs a SQL-level fix (join/subquery on each issue's newest signal level before `LIMIT`/`OFFSET`/`count()`), not a web-side workaround; (2) the deployed container (`docker compose`) was built before this fix pass — it needs a republish (`docker compose up -d --build` / `scripts/deploy-web.sh`) at the next phase boundary to pick up the terminal-breadcrumb, NaN-guard, appSlug, and sdk-chip changes; (3) `SIGNALS_PUBLIC_ADDRESS` (`apps/signals/src/environment.ts`, default `127.0.0.1:4640`) already backs the composed DSN (`apps/signals/src/dsn.ts`), but the web side never reads it: `apps-screen.tsx`'s "sending to {origin}" footer uses `window.location.origin` (the web app's own origin, not the collector's), and `new-app-dialog.tsx`'s `<script>` snippet hardcodes port `:4640` off `window.location.hostname` — both will silently point at the wrong host/port the day the collector's public address diverges from "web app host, port 4640"; worth reconciling once there's a real deployed host to point at.
