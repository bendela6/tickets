...
- **Expand/migrate/contract.** In this plan the schema stays **loose**: new columns `fields.ticket_type_id`, `fields.position`, `fields.required`, `link_types.ticket_type_id` are **nullable**; `fields.scheme_id` and `link_types.scheme_id` become **nullable but stay**; `ticket_type_fields` table **stays** (unused by the new seed). Plan C drops them and sets NOT NULL.
