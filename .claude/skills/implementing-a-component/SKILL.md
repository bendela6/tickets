...
4. Compose, don't configure: prefer composable named parts sharing implicit context over an ever-growing prop list; make elements polymorphic (an `as`/`asChild`-style mechanism) instead of duplicating wrapper components per element type.
...
6. Render every cell from the component's state manifest (see the mapping-component-states skill) in the workbench.
