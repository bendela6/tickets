...
/**
 * Exactly one of the two paint modes must be supplied — a discriminated union
 * rather than two optional fields, so the compiler (not a runtime assertion)
 * proves `sticks()` never reads an absent colour.
 */
type StickOpts =
  | (StickBase & { /** Literal stroke colour per stick. */ colours: [string, string, string]; classes?: undefined })
  | (StickBase & { /** Class per stick, for the favicon's media-query swap. */ classes: [string, string, string]; colours?: undefined });

/**
 * Sticks in reverse index order — SVG paints in document order and has no
 * z-index, so appending low, mid, top puts `top` frontmost.
 */
function sticks(opts: StickOpts): string {
  const { angles, reach, weight } = opts;
  let out = '';
  for (let i = 2; i >= 0; i--) {
    const paint = opts.classes ? ` class="${opts.classes[i]}"` : ` stroke="${opts.colours[i]}"`;
    out +=
...
/** Marks the two files committed straight from this module's output as generated, not hand-authored. */
