...
export function emitTones({ light }) {
  ...
  const { subtle, solid, outline } = emphasis;

  // A tone is a NAME for a scale. Roles come first so `primary` reads before
  ...
  // Nothing is emitted from the emphasis map any more — components spell the
  // rung where they build the class. It is still walked, because the check it
  // performs is about the RAMPS: every step a treatment reaches for has to
  // exist on every scale, or `bg-teal-3` is a class with no colour behind it.
  ...
  const roleNames = Object.keys(roles)
    .map((n) => `'${n}'`)
    .join(', ');
  // A tone name is not always a scale name (`primary` paints from `indigo`), so
