...
function GhostButton({ children }: { children: string }) {
  return (
    <button
      type="button"
      className="inline-flex h-36 items-center justify-center rounded-8 border-1 border-transparent px-14 font-sans text-13 font-500 text-gray-11 hover:bg-surface-inset hover:text-gray-12"
    >
      {children}
    </button>
  );
...
function PrimaryButton({ children }: { children: string }) {
  return (
    <button
      type="button"
      className="inline-flex h-36 items-center justify-center rounded-8 bg-indigo-9 px-14 font-sans text-13 font-500 text-indigo-contrast hover:bg-indigo-10"
    >
      {children}
    </button>
  );
