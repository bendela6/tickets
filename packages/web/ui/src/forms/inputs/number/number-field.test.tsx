...
function Controlled({ config = {} as NumberFieldConfig }) {
  const [value, setValue] = useState<number | null>(null);
  return <NumberField {...base} config={config} value={value} onChange={setValue} />;
}

describe('NumberField', () => {
  it('reports the number the user types', async () => {
    const onChange = vi.fn();
    render(<NumberField {...base} value={null} onChange={onChange} />);
  ...
  it('clamps a single entry above max down to max', async () => {
    const onChange = vi.fn();
    render(<NumberField {...base} config={{ max: 5 }} value={null} onChange={onChange} />);
  ...
  it('clamps a value below min up to min', async () => {
    const onChange = vi.fn();
    render(<NumberField {...base} config={{ min: 5 }} value={null} onChange={onChange} />);
  ...
  it('reports null when the field is cleared rather than coercing to zero', async () => {
    const onChange = vi.fn();
    render(<NumberField {...base} value={3} onChange={onChange} />);
  ...
  it('fires onBlur through the wrapper so the engine marks it touched', async () => {
    const onBlur = vi.fn();
    render(<NumberField {...base} onBlur={onBlur} value={null} onChange={vi.fn()} />);
  ...
  it('shows only what its parent gives it', async () => {
    render(<NumberField {...base} value={7} onChange={vi.fn()} />);
    const input = screen.getByRole('spinbutton');
  ...
  it('renders the suffix when configured', () => {
    render(<NumberField {...base} config={{ suffix: 'hours' }} value={null} onChange={vi.fn()} />);
    expect(screen.getByText('hours')).toBeInTheDocument();
  });

  it('is disabled when the engine says so', () => {
    render(<NumberField {...base} value={null} onChange={vi.fn()} disabled />);
