...
describe('SelectField', () => {
  it('shows the selected option', () => {
    render(<SelectField {...base} value="high" onChange={vi.fn()} />);
    expect(screen.getByText('High')).toBeInTheDocument();
  });

  it('reports the option the user picks', async () => {
    const onChange = vi.fn();
    render(<SelectField {...base} value={null} onChange={onChange} />);
  ...
  it('tolerates config whose options have not resolved yet', () => {
    render(<SelectField {...base} config={{}} value={null} onChange={vi.fn()} loading />);
    expect(screen.getByRole('button')).toBeDisabled();
  });

  it('is disabled while the engine resolves async config', () => {
    render(<SelectField {...base} value={null} onChange={vi.fn()} loading />);
    expect(screen.getByRole('button')).toBeDisabled();
  });

  it('is disabled when the engine says so', () => {
    render(<SelectField {...base} value={null} onChange={vi.fn()} disabled />);
    expect(screen.getByRole('button')).toBeDisabled();
  });
});
