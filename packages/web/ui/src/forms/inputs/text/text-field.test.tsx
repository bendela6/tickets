...
function Controlled({ config = {} as TextFieldConfig }) {
  const [value, setValue] = useState('');
  return <TextField {...base} config={config} value={value} onChange={setValue} />;
}
...
describe('TextField', () => {
  ...
  it('reports each keystroke to onChange', async () => {
    ...
    render(<TextField {...base} value="" onChange={onChange} />);
  ...
  it('shows only what its parent gives it', async () => {
    render(<TextField {...base} value="fixed" onChange={vi.fn()} />);
    const input = screen.getByRole('textbox');
  ...
  it('renders an undefined value as an empty controlled input', () => {
    render(<TextField {...base} value={undefined as unknown as string} onChange={vi.fn()} />);
    expect(screen.getByRole('textbox')).toHaveValue('');
  });

  it('fires onBlur so the engine can mark the field touched', async () => {
    const onBlur = vi.fn();
    render(<TextField {...base} onBlur={onBlur} value="" onChange={vi.fn()} />);
  ...
  it('marks itself invalid when the engine passes an error', () => {
    render(<TextField {...base} value="" onChange={vi.fn()} error="Required" />);
    expect(screen.getByRole('textbox')).toHaveAttribute('aria-invalid', 'true');
  });

  it('renders the prefix slot when configured', () => {
    render(<TextField {...base} config={{ prefix: 'https://' }} value="" onChange={vi.fn()} />);
    expect(screen.getByText('https://')).toBeInTheDocument();
  });

  it('passes the placeholder through', () => {
    render(<TextField {...base} config={{ placeholder: 'Ticket title' }} value="" onChange={vi.fn()} />);
    expect(screen.getByPlaceholderText('Ticket title')).toBeInTheDocument();
  });

  it('is disabled when the engine says so', () => {
    render(<TextField {...base} value="" onChange={vi.fn()} disabled />);
