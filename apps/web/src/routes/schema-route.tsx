...
import { EerCanvas, EerDiagramProvider, schemaGraphToModel } from '../components/eer';
...
function SchemaPage() {
  ...
  // and the mode panel, and the mode panel is where the database dropdown and
  // the diagram's outline live. Without it `/schema` renders a diagram with no
  // way to change what it is a diagram OF. No `h-screen` inside EerCanvas:
  // AppShell's <main> is already the height-constrained, `overflow-auto`
  // scroll container, so a second one inside it gives the page two scrollbars
  // and clips the diagram.
  //
  // The provider wraps the SHELL, not just the canvas: SchemaPanel renders the
  // outline (groups → tables, and the filter box over them) inside the mode
  // panel, so diagram state has to be in scope on both sides of <main>. It is
  // mounted unconditionally — with a null model while loading, failing, or
  // empty — so the shell never unmounts and remounts around a state change.
  return (
    <EerDiagramProvider model={result?.model ?? null}>
      <AppShell>
        ...
        {result?.model && drawable ? (
          <div className="flex h-full min-h-0 flex-col">
            {/* Warnings are how a MISSING edge explains itself. An fk pointing
                at a table this graph doesn't carry is skipped by
                deriveRelationships and only warned about by loadModel — so with
                nothing rendering them the user saw a line that simply wasn't
                there, with no way to tell a real absence from a bug. Collapsed
                by default: a healthy schema shouldn't be nagged about, and a
                broken one shouldn't cost the diagram half its height. */}
            ...
            {/* min-h-0 + flex-1 gives EerCanvas's own `h-full` a definite
                height to resolve against, so the banner takes space from the
                diagram instead of pushing it out of the shell's scroll
                container. */}
