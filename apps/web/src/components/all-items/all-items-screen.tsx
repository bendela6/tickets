...
import { Avatar, Button, cn, DialogContent, DialogRoot, DialogTitle, Icon, Input, ItemKey, Menu, MenuContent, MenuItem, MenuTrigger, Pill, RelativeDate, ScreenState, tableRender, Tabs, TONE_RAMP } from '@tickets/ui';
...
export function AllItemsScreen() {
  ...
  if (config.group === 'project') {
    ...
    for (const { kind, label } of KIND_ORDER) {
      // 11 is the text rung; the role goes through TONE_RAMP because `danger`
      // is a job, not a colour family.
      const textClass = `text-${TONE_RAMP[KIND_TONE[kind]]}-11`;
      const rows = allRows
