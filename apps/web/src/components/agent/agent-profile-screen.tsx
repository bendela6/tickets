...
export function AgentProfileScreen({ agentId }: { agentId: number }) {
  ...
  if (agents.isSuccess && !agent) {
    return (
      <div className="mx-auto max-w-4xl px-24 py-28">
  ...
  if (!agent) return <div className="px-24 py-28 font-sans text-12/17 text-gray-9">Loading…</div>;
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-24 px-24 py-28">
      <div className="flex items-start gap-16">
        <Avatar name={agent.name} {...avatarFor('agent')} size="md" className="size-44 text-15" />
        <div className="flex-1">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-8 font-mono text-12/17 text-gray-9">
            <span>
          ...
          <div className="overflow-hidden rounded-8 border-1 border-gray-6 bg-surface-raised">
            {mine.slice(0, 8).map((s) => (
              <Link
                ...
                className="flex items-center gap-12 border-b-1 border-gray-6 px-16 py-10 last:border-b-0 hover:bg-surface-inset"
              >
