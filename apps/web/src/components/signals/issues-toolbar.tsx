...
const selectClasses =
  'h-7 rounded-[7px] border border-gray-6 bg-surface-raised px-2 font-sans text-12 text-gray-11 ' +
        ...
        items={STATUSES.map((candidate) => ({
          ...
          label: (
            <>
              ...
              <span className="font-mono text-11 text-gray-9">{statusCounts[candidate] ?? ''}</span>
            </>
          ),
        }))}
        value={status}
