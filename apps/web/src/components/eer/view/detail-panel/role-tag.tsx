...
export function RoleTag({ role }: { role: 'pk' | 'fk' | null }) {
  if (!role)
    return <span className="inline-block w-28 shrink-0" aria-hidden />;
  return (
    <span
      className={cn('inline-block w-28 shrink-0 rounded-sm text-center font-mono text-9 font-600 leading-16', {
