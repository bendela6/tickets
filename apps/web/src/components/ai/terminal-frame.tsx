...
// Socket-connection state, distinct from the session's process status: it drives
// the title-bar indicator (live / reconnecting / ended) and whether Restart is
// offered. 'ended' covers exited/interrupted/failed — the process is gone.
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-card border border-hairline bg-raised">
      ...
      {ended || truncated ? (
        <div className="flex items-center gap-3 border-t border-hairline px-3.5 py-2 font-sans text-meta">
          ...
          {ended && onRestart ? (
            <Button size="compact" variant="secondary" onClick={onRestart}>
              Restart
            </Button>
          ) : null}
        </div>
