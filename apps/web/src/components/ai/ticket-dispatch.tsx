...
// dispatches a persona onto it. Closes the loop — the ticket board becomes an
// agent work queue.
export function TicketDispatch({ ticketId, actorId }: { ticketId: number; actorId?: number }) {
  ...
  // The most recent still-live session working this ticket.
  const live = (sessions.data ?? [])
    .filter((s) => s.ticketId === ticketId && LIVE.includes(s.status))
  ...
  async function submit() {
    ...
    try {
      ...
      setOpen(false);
      setPrompt('');
      void navigate({ to: '/ai/$sessionId', params: { sessionId: String(session.id) } });
    } catch (err) {
