...
import type { AiAgent, AiProvider, PermissionMode } from '../../api/types';
import { useAiWorkspaces } from '../../api/use-ai-workspaces';
import { useCreateAiAgent } from '../../api/use-create-ai-agent';
import { usePatchAiAgent } from '../../api/use-patch-ai-agent';
...
const NEEDS_PERMISSIONS: PermissionMode[] = ['default', 'acceptEdits', 'dontAsk', 'auto'];
...
export function AgentEditor({
  ...
  const [workspaceId, setWorkspaceId] = useState<string | null>(
    agent?.defaultWorkspaceId != null ? String(agent.defaultWorkspaceId) : null,
  );
  ...
  async function save() {
    ...
    const allowedTools = tools
      ...
      .map((t) => t.trim())
      .filter(Boolean);
    const workspace = workspaceId != null ? Number(workspaceId) : null;
    try {
      if (editing) {
        ...
        await create.mutateAsync({
          ...
          allowedTools,
          permissionMode,
          defaultWorkspaceId: workspace ?? undefined,
        });
      }
      onOpenChange(false);
    } catch (err) {
      ...
      <DialogContent className="w-[min(34rem,calc(100vw-2rem))]">
        ...
        <div className="mt-4 flex max-h-[70vh] flex-col gap-4 overflow-y-auto pr-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block">
              <Label hint="optional">Default workspace</Label>
              <Combobox
