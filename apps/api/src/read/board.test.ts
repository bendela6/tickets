...
import { linkTypeSetTargetTypes } from '../command/config/link-type';
...
// Regression for the task-13 lesson: link_type_target_types rows must survive
// a board read, or the web chip editor would start empty and a toggle would
// silently wipe a link type's existing targets via the full-replace PUT.
it('returns the link_type_target_types rows so a chip editor can seed from them', async () => {
  const fx = await seedFixture();
  ...
  const bugId = fx.typeIdByKey.get('bug')!;
  await runCommand(testDb, linkTypeSetTargetTypes, { commandId: crypto.randomUUID(), actorId: fx.actorId }, {
    linkTypeId: blocks.id, targetTypeIds: [taskId, bugId],
  });
  const board = await buildBoard(testDb, fx.projectKey);
  const rows = board.targetTypes.filter((row) => row.linkTypeId === blocks.id);
  expect(rows.map((row) => row.targetTypeId).sort((a, b) => a - b)).toEqual([taskId, bugId].sort((a, b) => a - b));
});
