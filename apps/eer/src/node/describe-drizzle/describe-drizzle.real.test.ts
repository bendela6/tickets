...
it('describes the real 29-table schema with no unsupported constructs', () => {
  const d = describeDrizzle(schema as Record<string, unknown>, []);
  // 29 = the items platform's 22 (still public until Plan 2) + core.workdirs
  // + the 2 terminal.* tables + the 4 agent.* tables. The 5 legacy public ai_*
  // tables were deleted in Task 11 once nothing read them.
  expect(d.tables).toHaveLength(29);
  ...
  // 22 = 15 + core.workdirs' serial id + the 2 terminal.* serial ids + the 4
