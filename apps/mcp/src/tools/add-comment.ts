...
import { encodeBody } from '../helpers/rich-content';
...
export function registerAddComment(server: McpServer, context: ToolContext) {
  server.registerTool(
    ...
    {
      description:
        'Add a comment to a ticket, attributed to the configured actor. Body is markdown by default; pass format: "rich" to submit a serialized tiptap doc verbatim.',
      inputSchema: {
        projectKey: z.string(),
    ...
    ({ projectKey, ticketNumber, body, format }) =>
      runTool('add_comment', async () => {
        ...
        await apiFetch(`/api/tickets/${ticket.id}/comments`, {
          method: 'POST',
          body: JSON.stringify({ authorId: actorId, body: encodeBody(body, format ?? 'markdown') }),
        });
